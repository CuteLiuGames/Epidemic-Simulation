...
#建立圖片
number = 0
plt.savefig(f'out2-4/model_{number}')
plt.close()
  ...
  for i in range (m):
   for j in range (n):
    for k in range(human_number):
      ...
      if (human_x[total]+vx[total] * rx[total]) >1+1.2*i-0.05 and rx[total] > 0:
         rx[total] = 0
      if (human_x[total]+vx[total] * rx[total]) <0+1.2*i+0.05 and rx[total] < 0:
         rx[total] = 0
      if (human_y[total]+vy[total] * ry[total]) >1+1.2*j-0.05 and ry[total] > 0:
         ry[total] = 0
      if (human_y[total]+vy[total] * ry[total]) <0+1.2*j+0.05 and ry[total] < 0:
         ry[total] = 0
...
for i in range(number+1):
    image = imageio.imread(f'out2-4/model_{i}.png')
    images_.append(image)
    
imageio.mimsave('out_model_2-4.gif',images_,'GIF',duration=0.05)
print('Done!')