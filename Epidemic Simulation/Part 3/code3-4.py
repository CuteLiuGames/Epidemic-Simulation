...
#建立圖片
number = 0
plt.savefig(f'out3-4/model_{number}')
plt.close()
...
for step in range(times):
  ...
  for i in range (m):
   for j in range (n):
    for k in range(human_number):
      ...
      total+=1
      rx[total] = random.randint(-1, 1)
      ry[total] = random.randint(-1, 1)
      if (human_x[total]+vx[total] * rx[total]) >1+1.2*mi[total]-0.05 and rx[total] > 0:
         rx[total] = 0
      if (human_x[total]+vx[total] * rx[total]) <0+1.2*mi[total]+0.05 and rx[total] < 0:
         rx[total] = 0
      if (human_y[total]+vy[total] * ry[total]) >1+1.2*nj[total]-0.05 and ry[total] > 0:
         ry[total] = 0
      if (human_y[total]+vy[total] * ry[total]) <0+1.2*nj[total]+0.05 and ry[total] < 0:
         ry[total] = 0
      human_x[total] += vx[total] * rx[total]
      human_y[total] += vy[total] * ry[total]
  ...
  for i in range (m):
   for j in range (n):
    for k in range(human_number):
      total+=1
      if random.random() <= Travel_rate and isolated[total] == 0:
  ...
  for i in range(n*m*human_number):
        for j in range(n*m*human_number):
            if distance(human_x[i], human_y[i], human_x[j], human_y[j]) <= infectious_distance and isolated[total] == 0:
  ...
  for i in range (m):
   for j in range (n):
    for k in range(human_number):
      total+=1
      if si[total] == 1:
          if random.random() <= Isolation_rate and si_days[total] >= random.randint(3, 14) and isolated[total] == 0:
            isolated[total] = 1
            mi[total] = m
            nj[total] = n
            human_x[total] = (random.randint(10, 90)/100+1.2*mi[total])
...
for i in range(number+1):
    image = imageio.imread(f'out3-4/model_{i}.png')
    images_.append(image)
    
imageio.mimsave('out_model_3-4.gif',images_,'GIF',duration=0.05)
print('Done!')